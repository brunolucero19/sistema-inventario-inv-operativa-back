        ...
        if (hoy >= proximaRevision) {
            ...
            const ordenCompra = await prisma.ordenCompra.findMany({
                where: {
                    ...
                    estadoOrdenCompra: {
                        id_estado_orden_compra: {OR: [estadosOC.pendiente, estadosOC.enviada]}
                    },
                    include: {
                        modeloInventario: true,
                    }
            ...
            const stock_total = stock + ordenCompra.reduce((total, oc) => total + oc.cantidad, 0);
            
            const cantidadAReponer = Math.max(0, calcularStockAReponer(demanda_diaria, periodo_revision, pa.demora_entrega, nivelServicioZ[pa.nivel_servicio], desviacion_est_dem, stock_total));
            console.log('cantidad a reponer', calcularStockAReponer(demanda_diaria, periodo_revision, pa.demora_entrega, nivelServicioZ[pa.nivel_servicio], desviacion_est_dem, stock_total))
            console.log(`[${hoy.toISOString()}] Artículo: ${pa.articulo.descripcion} - Reposición: ${cantidadAReponer} uds (Stock actual: ${stock_total}, Demanda diaria: ${demanda_diaria.toFixed(2)})`);
